...
def change_last_element_of_row(dataset):
    for  row in dataset:
        if(row[-1]<5):
            row[-1]=0
        else:
...
def get_rid_of_least_important_column(set,id):

    new_set=[[row[i] for i in range(len(row)) if i!=id] for row in set]
...
if __name__ == '__main__':
    ...
    x = int(input())
    dataset=change_last_element_of_row(dataset)


    train_set=dataset[int(len(dataset)*(x/100)):]
    train_set_x=[row[:-1] for row in train_set]
    train_set_y=[row[-1] for row in train_set]


    test_set=dataset[:int(len(dataset)*(x/100))]
    ...
    importantance =list(classifier1.feature_importances_)
    indexOfLeastImportantElement = importantance.index(min(importantance))


    new_train_x=get_rid_of_least_important_column(train_set_x,indexOfLeastImportantElement)
    new_test_x=get_rid_of_least_important_column(test_set_x,indexOfLeastImportantElement)

    classifier2=MLPClassifier(15,activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    scaler1=StandardScaler()
    scaler1.fit(new_train_x)

    scaler2=MinMaxScaler()
    scaler2.fit(new_train_x)


    classifier2.fit(scaler1.transform(new_train_x), train_set_y)
    accuracy1 = calculate_accuracy(classifier2.predict(scaler1.transform(new_test_x)),test_set_y)
    print(f"Accuracy with StandardScaler: {accuracy1}")

    classifier2.fit(scaler2.transform(new_train_x),train_set_y)
    accuracy1 = calculate_accuracy(classifier2.predict(scaler2.transform(new_test_x)),test_set_y)
    print(f"Accuracy with MinMaxScaler: {accuracy1}")
