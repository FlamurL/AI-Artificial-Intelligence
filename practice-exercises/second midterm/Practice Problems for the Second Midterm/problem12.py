...
if __name__ == '__main__':
    ...
    classifier=MLPClassifier(max_iter=20,activation='relu', random_state=0, hidden_layer_sizes=neurons, learning_rate_init=learningRate)
    classifier.fit(scaler.transform(train_set_x),train_set_y)

    accuracy_train = accuracy(train_set_y, classifier.predict(scaler.transform(train_set_x)))
    accuracy_val = accuracy(test_set_y, classifier.predict(scaler.transform(test_set_x)))

    if accuracy_train > accuracy_val * 1.15:
        train_X_new = removed_column_set(train_set_x, idx)
        scaler.fit(train_X_new)
        classifier.fit(scaler.transform(train_X_new), train_set_y)
        sample = [sample[i] for i in range(0, len(sample)) if i != idx]
